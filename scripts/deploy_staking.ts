...
async function main() {
  ...
  switch(network){
    ...
    case "localhost":
      ...
      staking = await deploy("Staking", [token.address]);
    ...
    case "rinkeby":
      ...
      staking = await deploy("VITAStaking", [tokenAddress]);
      contracts.push(staking);
      break;
    // mainnet deploy
    case "mainnet":
      tokenAddress = process.env.MAINNET_VITA_ADDRESS;
      staking = await deploy("Staking", [tokenAddress]);
      contracts.push(staking);
      break;
    default:
      console.log(chalk.magenta("Please switch network to localhost, rinkeby, or mainnet"));
  }
